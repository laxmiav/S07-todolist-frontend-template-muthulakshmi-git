const task = {
  ...
  setStatusComplete: function(event) {
    ...
    // STEP episode 6
    // sauvegarde du niveau de completion en faisant un appel vers l'api
    task.saveCompletion(taskElement, 100);
  },
  ...
  updateTitle: function(event) {
    ...
    task.saveTitle(taskElement, taskName);
  },
  ...
  saveTitle: function(taskElement, newTitle) {
    ...
    const ajaxOptions = {
      method: 'PATCH', // nous allons faire un appel "patch" pour seulement mettre à jour le nom de la tâche
      headers: httpHeaders,
    ...
    const url = task.apiRootURL + '/tasks/' + taskElement.dataset.id;
    // STEP episode 6
  ...
  saveCompletion: function(taskElement, completion) {
    ...
    const ajaxOptions = {
      method: 'PATCH', // nous allons faire un appel "patch" pour seulement mettre à jour le taux de completion
      headers: httpHeaders,
